...
parameters['atomorder'] = "C,C_alpha,C_beta"
...
analysis = AngularCorrelation_serial(parameters)
analysis.runAnalysis()

parameters['atomorder'] = "C,C_alpha,C_beta"
parameters['pyroserver'] = "multiprocessor:2"
parameters['output'] = parallel
analysis = AngularCorrelation_parallel(parameters)
analysis.runAnalysis()

class Test_Serial_vs_Parallel(unittest.TestCase):
    
    def check_var(self, varName, serialVal, parallelVal):
                                                            
        errorMax = max(abs(N.ravel(serialVal - parallelVal)))
